...
export function TerminalContact({ onClose }: TerminalContactProps) {
  ...
  const [history, setHistory] = useState<{ id: number; type: 'system' | 'user'; text: string; isError?: boolean; speed?: number }[]>([
    { id: 1, type: 'system', text: 'ctOS v2.0.4 // INITIALIZING SECURE UPLINK...', speed: 5 },
    ...
    if (e.key === 'Enter' && inputValue.trim() && !isTyping) {
      ...
      if (step === 'name') {
        ...
        try {
          ...
          setTimeout(() => {
            addSystemMessage('FATAL ERR: TRANSMISSION BLOCKED BY ctOS FIREWALL.', true, 5);
            setStep('error');
          }, 1000);
        }
      }
  ...
  return (
    ...
    >
      <motion.div 
        ...
        className={`w-full max-w-3xl bg-black border-2 ${step === 'error' ? 'border-red-500 shadow-[0_0_30px_rgba(239,68,68,0.4)]' : 'border-cyan-500 shadow-[0_0_30px_rgba(34,211,238,0.2)]'} flex flex-col relative`}
      ...
      >
        ...
        <div className={`flex items-center justify-between px-4 py-2 ${step === 'error' ? 'bg-red-500 text-black' : 'bg-cyan-500 text-black'} relative z-30 font-bold tracking-widest uppercase text-xs sm:text-sm`}>
          <div className="flex items-center gap-3">
            ...
            <span>ctOS // ROOT_ACCESS_GRANTED</span>
          </div>
        ...
        {/* Terminal Body */}
        <div 
          className="p-6 h-[400px] sm:h-[450px] overflow-y-auto font-mono text-sm sm:text-base flex flex-col relative z-10 scrollbar-thin scrollbar-thumb-cyan-500/30 scrollbar-track-transparent"
        ...
        >
          {history.slice(0, visibleLines).map((line, i) => (
            ...
            >
              {line.type === 'system' ? (
                <>
                  <span className="text-white/30 shrink-0 hidden sm:block">ctOS&gt;</span>
                  <TypewriterLine 
                    text={line.text} 
        ...
        <div className="absolute inset-0 pointer-events-none bg-[url('/src/assets/scanline-overlay.svg')] opacity-30 mix-blend-overlay z-0"></div>
        <div className="absolute inset-0 pointer-events-none bg-[linear-gradient(rgba(0,255,255,0.03)_1px,transparent_1px),linear-gradient(90deg,rgba(0,255,255,0.03)_1px,transparent_1px)] bg-[size:20px_20px] z-0"></div>
        
        {/* Glitch Line Effect */}
        <motion.div 
          animate={{ top: ['-10%', '110%'] }}
          transition={{ duration: 3, repeat: Infinity, ease: "linear", repeatDelay: 2 }}
          className="absolute left-0 right-0 h-8 bg-gradient-to-b from-transparent via-cyan-500/10 to-transparent pointer-events-none z-20"
        />
      </motion.div>
    </motion.div>
  );
